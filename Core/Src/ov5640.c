...
static int32_t OV5640_IO_ReadReg(uint16_t addr, uint16_t reg, uint8_t *pData, uint16_t Length);
static int32_t OV5640_IO_GetTick(void);
static int32_t OV5640_IO_Init(void)
{
    /* I2C 已在 MX_I2C2_Init() 中初始化，这里无需重复 */
    return OV5640_OK;
}
...
static int32_t OV5640_IO_WriteReg(uint16_t addr, uint16_t reg, uint8_t *pData, uint16_t Length)
{
    /* 使用 HAL I2C 写，注意地址是 7 位，寄存器 16 位 */
...
static int32_t OV5640_IO_ReadReg(uint16_t addr, uint16_t reg, uint8_t *pData, uint16_t Length)
{
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c2, addr, reg, I2C_MEMADD_SIZE_16BIT, pData, Length, 100);
    return (status == HAL_OK) ? OV5640_OK : OV5640_ERROR;
}
...
static int32_t OV5640_ModifyRegWrap(void *handle, uint16_t Reg, uint16_t Mask, uint8_t *Data, uint16_t Length);
...
int32_t OV5640_RegisterBusIO(OV5640_Object_t *pObj, OV5640_IO_t *pIO)
{
  ...
  else
  {
    ...
    pObj->Ctx.ModifyReg = OV5640_ModifyRegWrap;
  ...
  if (pObj->IO.Init == NULL) 
      {
        ...
        pObj->IO.Address   = OV5640_I2C_ADDR; // 需定义为 0x3C
...
int32_t OV5640_SetBrightness(OV5640_Object_t *pObj, int32_t Level)
{
  ...
  if (ret == OV5640_OK)
  {
    ...
    {
      tmp = 0x08;
      if (ov5640_modify_reg(&pObj->Ctx, OV5640_SDE_CTRL8, mask, &tmp, 1) != OV5640_OK)
      {
        ret = OV5640_ERROR;
      }
    }
...
int32_t OV5640_SetSaturation(OV5640_Object_t *pObj, int32_t Level)
{
  ...
  {
    tmp = 0x41;
    ret = ov5640_modify_reg(&pObj->Ctx, OV5640_SDE_CTRL8, mask, &tmp, 1);
  }
...
/**
  * @brief  Wrap component ModifyReg to Bus Modify function
  * @param  handle  Component object handle
  * @param  Reg  The target register address to write
  * @param  pData  The target register value to be written
  * @param  Length  buffer size to be written
  * @retval error status
  */
static int32_t OV5640_ModifyRegWrap(void *handle, uint16_t Reg, uint16_t Mask, uint8_t *pData, uint16_t Length)
{
  OV5640_Object_t *pObj = (OV5640_Object_t *)handle;
  uint8_t TempData1 = 0;

 pObj->IO.ReadReg(pObj->IO.Address, Reg, &TempData1, Length);


 TempData1 = TempData1 | ((*pData) & Mask);

 return pObj->IO.WriteReg(pObj->IO.Address, Reg, &TempData1, Length);
}
